...
export default function HomePage() {
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      {/* Hero */}
      <section className="relative mb-10 overflow-hidden rounded-3xl border border-border bg-gradient-to-br from-[#1a0b2e] via-[#140a22] to-[#0b0614] p-8 sm:p-12">
        ...
        <div className="relative z-10 max-w-2xl">
          ...
          <div className="flex flex-wrap gap-3">
            <Link
              href="/catalog"
              className="rounded-xl bg-gradient-to-r from-primary to-primary-hover px-6 py-3 text-sm font-semibold text-white transition hover:opacity-90 glow-primary"
            >
              Смотреть каталог
            </Link>
            ...
            <Link
              href="/sell"
              className="rounded-xl border border-border bg-card px-6 py-3 text-sm font-semibold text-foreground transition hover:border-primary/50"
            >
              Начать продавать
            </Link>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 md:grid-cols-8">
          {popularGames.map((game) => (
            ...
            >
              ...
              <span className="text-center text-xs font-medium text-foreground transition group-hover:text-primary">
                {game.name}
              </span>
            </Link>
          ))}
